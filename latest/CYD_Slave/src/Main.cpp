 ...
 * Version: 3.0 (NASA API Integration & APOD Screensaver)
 */
#include <Arduino.h>
#include <TFT_eSPI.h>
#include <SPI.h>
...
Preferences prefs;
Lx200 mount;
enum Page { BOOT,HOME,CATALOG,CATALOG_MODE,CONFIRM,SETTINGS,NIGHT,LOCATION,CALIBRATE,QUICKMODE,SET_TIMER,TRACKING,HOMING,NASA_MENU,NASA_LOADING,APOD_VIEW };
Page page=BOOT;
enum TrackSource { SRC_CATALOG, SRC_APP, SRC_NASA };
TrackSource currentTrackSource = SRC_CATALOG;
double nasaTargetRa = 0;
double nasaTargetDec = 0;
...
void tap(int x,int y) {
  ...
  if(page==HOME && y<60) {
    ...
    else if(x<210) { changePage(CATALOG_MODE); }
    ...
    if(y>=50 && y<=90) { // Tapped top
      ...
      if(x>=40 && x<=280) {
        ...
        if(currentTrackSource == SRC_CATALOG && selected >= 0) {
          ...
          Serial2.println(cmd);
        } else if(currentTrackSource == SRC_NASA) {
          String cmd = "T:" + String(nasaTargetRa, 5) + "," + String(nasaTargetDec, 5);
          Serial2.println(cmd);
  ...
  } else if(page==CATALOG_MODE) {
    if(y>=70 && y<110) { changePage(SET_TIMER); currentTrackSource = SRC_APP; } // Stellarium WiFi
    else if(y>=120 && y<160) { changePage(NASA_MENU); }
    else if(y>=170 && y<210) { changePage(CATALOG); }
  } else if(page==NASA_MENU) {
    String cmdID = "";
    if(y>=70 && y<110) cmdID = "301"; // Moon
    else if(y>=120 && y<160) cmdID = "499"; // Mars
    else if(y>=170 && y<210) cmdID = "599"; // Jupiter
    else if(y>=220 && y<260) { nasaApi.apodFetched = false; changePage(APOD_VIEW); return; } // APOD
    
    if(cmdID != "") {
       changePage(NASA_LOADING);
       // We must force screen update before freezing for HTTP
       extern void loop();
       // Actually, we can't easily force loop here. It's better to just let it happen in loop(), but tap() blocks.
       // We will just let it freeze, it's fast enough.
       auto eph = nasaApi.getPlanetEphemeris(cmdID);
       if(eph.valid) {
          nasaTargetRa = eph.raHours;
          nasaTargetDec = eph.decDeg;
          currentTrackSource = SRC_NASA;
          changePage(SET_TIMER);
       } else {
          changePage(NASA_MENU);
       }
    }
  } else if(page==APOD_VIEW) {
    changePage(NASA_MENU);
...
void drawCatalogMode() {
  backHeader("SELECT TARGET SOURCE");
  // Centered: (320 - 260) / 2 = 30
  button("1. APP (STELLARIUM WIFI)", 30, 70, 260, 40, ink());
  button("2. NASA HORIZONS (LIVE)", 30, 120, 260, 40, ink());
  button("3. OFFLINE (INTERNAL)", 30, 170, 260, 40, ink());
}

void drawNasaMenu() {
  backHeader("NASA HORIZONS & APOD");
  button("MOON (Real-time)", 15, 70, 140, 40, ink());
  button("MARS (Real-time)", 165, 70, 140, 40, ink());
  button("JUPITER (Real-time)", 15, 120, 290, 40, ink());
  button("NASA APOD (SCREENSAVER)", 15, 170, 290, 40, orange());
}

void drawNasaLoading() {
  canvas.fillScreen(TFT_BLACK);
  canvas.setTextDatum(MC_DATUM);
  canvas.setTextColor(white(), TFT_BLACK);
  canvas.drawString("Fetching from NASA...", 160, 120, 2);
  canvas.setTextDatum(TL_DATUM);
}

void drawApodView() {
  // We leave this mostly blank because nasaApi.drawAPOD() draws directly to tft,
  // bypassing the canvas banding to save RAM.
  // We just put a fallback text here in case drawing failed or during bands.
  if(!nasaApi.apodFetched && bandY == 0) {
    nasaApi.drawAPOD();
  }
}
...
void render() {
  ...
  if (page == APOD_VIEW) {
     // Stream JPEG directly to TFT once, do not use bands
     if(!nasaApi.apodFetched) {
        nasaApi.drawAPOD();
     }
     // Keep polling touch so we can exit
     pollTouch();
     return;
  }
  ...
  switch(page) {
    ...
    case CATALOG_MODE:drawCatalogMode();break;
    case NASA_MENU:drawNasaMenu();break;
    case NASA_LOADING:drawNasaLoading();break;
    case APOD_VIEW:drawApodView();break;
