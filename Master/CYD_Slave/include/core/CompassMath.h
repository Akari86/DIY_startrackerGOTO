...
inline Point projectLocal(double right, double forward, double up) {
  // An oblique view keeps all three rings visible at a level orientation.
  double y = forward * 0.8660254037844386 - up * 0.5;
  double depth = forward * 0.5 + up * 0.8660254037844386;
  double perspective = 2.8 / (2.8 + depth);
  return {80 + (int)std::lround(40 * right * perspective), 147 - (int)std::lround(40 * y * perspective), depth};
}
